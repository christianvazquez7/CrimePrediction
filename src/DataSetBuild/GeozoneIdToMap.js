...
client.connect(NoSQLconnection, function (err, theDb) {
    	...
    	var csvStream = csv.createWriteStream({headers: true}),
   		writableStream = fs.createWriteStream("../../data/Quarter/zed-n-r.csv");
		...
		theDb.collection('Geozone').find({}).toArray(function(error,result){
			...
			csv.fromPath("../../data/Quarter/zed-n.csv").on("data", function(data){
				...
				if(geo != undefined) {
					...
					csvStream.write({ gridID: data[0], day_name: data[1], month_name: data[2], date: data[3], year: data[4], hour: data[5], lat: lati, lon: loni, expected: data[8]});
