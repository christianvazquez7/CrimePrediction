...
years = ['2006','2007','2008','2009','2011','2012','2013','2014','2015']
i = 0
num_of_features = 18
# train_set = []
# for j in range(0,10):
...
test = unbalancedDataFromYears(['2010'])
X_test = test[:,0:18]
...
cv_rec_weighted = map(lambda tp: sklearn.metrics.recall_score(tp[0], tp[1], average='weighted'), ys)
cv_f1_weighted = map(lambda tp: sklearn.metrics.f1_score(tp[0], tp[1], average='weighted'), ys)
cv_auc_weighted = map(lambda tp: sklearn.metrics.roc_auc_score(tp[0], tp[1]), yscc)

cv_pr_binary = map(lambda tp: sklearn.metrics.precision_score(tp[0], tp[1], average='binary'), ys)
cv_rec_binary = map(lambda tp: sklearn.metrics.recall_score(tp[0], tp[1], average='binary'), ys)
cv_f1_binary = map(lambda tp: sklearn.metrics.f1_score(tp[0], tp[1], average='binary'), ys)

cv_pr_micro = map(lambda tp: sklearn.metrics.precision_score(tp[0], tp[1], average='micro'), ys)
cv_rec_micro = map(lambda tp: sklearn.metrics.recall_score(tp[0], tp[1], average='micro'), ys)
cv_f1_micro = map(lambda tp: sklearn.metrics.f1_score(tp[0], tp[1], average='micro'), ys)

cv_pr_macro = map(lambda tp: sklearn.metrics.precision_score(tp[0], tp[1], average='macro'), ys)
cv_rec_macro = map(lambda tp: sklearn.metrics.recall_score(tp[0], tp[1], average='macro'), ys)
cv_f1_macro = map(lambda tp: sklearn.metrics.f1_score(tp[0], tp[1], average='macro'), ys)

# i = i + 1

print "------------------------------"

print "Result for YEAR: "
...
print {'CV accuracy': np.mean(cv_acc), 'CV precision_weighted': np.mean(cv_pr_weighted),
        'CV recall_weighted': np.mean(cv_rec_weighted), 'CV F1_weighted': np.mean(cv_f1_weighted), 'AUC': np.mean(cv_auc_weighted), 'precision_binary': np.mean(cv_pr_binary), 'recall_binary':np.mean(cv_rec_binary),'f1_binary':np.mean(cv_f1_binary)
        ,'precision_macro': np.mean(cv_pr_macro), 'recall_macro':np.mean(cv_rec_macro),'f1_macro':np.mean(cv_f1_macro), 'precision_micro': np.mean(cv_pr_micro), 'recall_micro':np.mean(cv_rec_micro),'f1_micro':np.mean(cv_f1_micro)}
