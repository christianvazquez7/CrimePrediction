...
client.connect(NoSQLconnection, function (err, theDb) {
	...
	var ac = {};
	csv.fromPath("../../data/balanced/i2006.csv")
	...
	.on("end", function(){
		...
		theDb.collection('Geozone').update({}},{$set: {level:1}},{upsert:false, multi:true}, function(x, o){

			for (key in ac) {
				//console.log('hereee');

				theDb.collection('Geozone').update({zone_id:parseInt(key)},{$set: {level:10}},{upsert:false, multi:true}, function(x, o){
						if(!x){
							console.log(o);
							console.log('Updated!');
						} else {
