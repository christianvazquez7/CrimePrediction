...
module.exports = function Census() { 
    ...
    var $ = require("jquery")(jsdom.jsdom().defaultView), XMLHttpRequest = require('xmlhttprequest').XMLHttpRequest;
    $.support.cors = true;
    ...
    var API_URL = "http://censusdata.ire.org" // can be overridden
    var GEOAPI_URL = "http://census.ire.org/geo/1.0"  // can be overridden
    ...
    this.getData = function(geoid, handler) {
    ...
    this.getGeo = function(point,success_handler,options) {
        if ($.isArray(point)) {
            point = point.join(',');
        }
